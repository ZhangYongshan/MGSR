...
function [acc_O, NMI_sqrt, Purity, kappa] = evaluate_results_clustering(tlabs, Trlabs)
...
Trlabs = Trlabs(:);
tlabs = tlabs(:);
if size(Trlabs) ~= size(tlabs)
    error('size(Trlabs) must == size(tlabs)');
end

Label = unique(Trlabs);
nClass = length(Label);

Labetlabs = unique(tlabs);
nClass2 = length(Labetlabs);
if nClass2 < nClass
     % smooth
     Trlabs = [Trlabs; Label];
     tlabs = [tlabs; Label];
elseif nClass2 > nClass
     % smooth
     Trlabs = [Trlabs; Labetlabs];
     tlabs = [tlabs; Labetlabs];
end

G = zeros(nClass);
for i=1:nClass
    for j=1:nClass
        G(i,j) = sum(Trlabs == Label(i) & tlabs == Label(j));
    end
end
sumG = sum(G(:));
